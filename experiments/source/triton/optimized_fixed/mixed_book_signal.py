...
@triton.jit
def mixed_book_signal_kernel(out_ptr, n, bid_ptr, ask_ptr, scale, epsilon, threshold, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def run(scale: float, epsilon: float, threshold: float, bid: torch.Tensor, ask: torch.Tensor) -> torch.Tensor:
    ...
    mixed_book_signal_kernel[
        grid
    ](out, bid.numel(), bid, ask, float(scale), float(epsilon), float(threshold), BLOCK_SIZE=BLOCK_SIZE, num_warps=NUM_WARPS)
    return out
