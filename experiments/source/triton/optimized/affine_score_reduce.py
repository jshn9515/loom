...
@triton.jit
def affine_score_reduce_kernel(partial_ptr, n, size_bucket, x_ptr, scale, bias, threshold, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def run(scale: float, bias: float, threshold: float, x: torch.Tensor) -> torch.Tensor:
    ...
    active = triton.cdiv(x.numel(), selected_block_size(affine_score_reduce_kernel, REDUCE_MIN_BLOCK_SIZE))
    return reduce_sum(partial[:active])


def autotune_state() -> dict[str, object]:
    return collect_autotune_state({"affine_score_reduce_kernel": affine_score_reduce_kernel, "reduce_sum_kernel": reduce_sum_kernel})
