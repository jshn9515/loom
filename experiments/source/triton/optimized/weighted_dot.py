...
@triton.jit
def weighted_dot_kernel(partial_ptr, n, size_bucket, x_ptr, y_ptr, weight, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def run(weight: float, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    weighted_dot_kernel[grid](partial, x.numel(), size_bucket(x.numel()), x, y, float(weight))
    active = triton.cdiv(x.numel(), selected_block_size(weighted_dot_kernel, REDUCE_MIN_BLOCK_SIZE))
