...
@triton.jit
def mse_sum_kernel(partial_ptr, n, size_bucket, x_ptr, y_ptr, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def run(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    mse_sum_kernel[grid](partial, x.numel(), size_bucket(x.numel()), x, y)
    active = triton.cdiv(x.numel(), selected_block_size(mse_sum_kernel, REDUCE_MIN_BLOCK_SIZE))
