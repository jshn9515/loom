...
@triton.jit
def signal_clip_reduce_kernel(partial_ptr, n, size_bucket, bid_ptr, ask_ptr, scale, epsilon, clip, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def run(scale: float, epsilon: float, clip: float, bid: torch.Tensor, ask: torch.Tensor) -> torch.Tensor:
    ...
    partial = torch.zeros((triton.cdiv(bid.numel(), REDUCE_MIN_BLOCK_SIZE),), device=bid.device, dtype=torch.float32)
    ...
    active = triton.cdiv(bid.numel(), selected_block_size(signal_clip_reduce_kernel, REDUCE_MIN_BLOCK_SIZE))
