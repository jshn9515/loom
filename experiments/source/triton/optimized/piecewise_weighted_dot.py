...
@triton.jit
def piecewise_weighted_dot_kernel(partial_ptr, n, size_bucket, x_ptr, y_ptr, weight_pos, weight_neg, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def run(weight_pos: float, weight_neg: float, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    active = triton.cdiv(x.numel(), selected_block_size(piecewise_weighted_dot_kernel, REDUCE_MIN_BLOCK_SIZE))
