...
__global__ void mixed_weighted_affine_dot_partial_kernel(const float* x, const float* y, float weight,
                                                         float scale, float bias, float* partials,
                                                         std::int64_t n) {
...
extern "C" int mixed_weighted_affine_dot_run(float weight, float scale, float bias, const float* x,
                                             const float* y, float* out, std::int64_t n) {
  if (n <= 0) {
  ...
  mixed_weighted_affine_dot_partial_kernel<<<blocks, kBlockSize>>>(x, y, weight, scale, bias, partials, n);
