...
__global__ void mse_partial_kernel(const float* x, const float* y, float* partials, std::int64_t n) {
...
extern "C" int mse_sum_run(const float* x, const float* y, float* out, std::int64_t n) {
  if (n <= 0) {
  ...
  mse_partial_kernel<<<blocks, kBlockSize>>>(x, y, partials, n);
