...
__global__ void huber_partial_kernel(const float* x, const float* y, float delta, float* partials,
                                     std::int64_t n) {
...
extern "C" int huber_sum_run(float delta, const float* x, const float* y, float* out, std::int64_t n) {
  ...
  huber_partial_kernel<<<blocks, kBlockSize>>>(x, y, delta, partials, n);
