...
__global__ void piecewise_weighted_dot_partial_kernel(const float* x, const float* y, float weight_pos,
                                                      float weight_neg, float* partials, std::int64_t n) {
...
extern "C" int piecewise_weighted_dot_run(float weight_pos, float weight_neg, const float* x, const float* y,
  ...
  piecewise_weighted_dot_partial_kernel<<<blocks, kBlockSize>>>(x, y, weight_pos, weight_neg, partials, n);
