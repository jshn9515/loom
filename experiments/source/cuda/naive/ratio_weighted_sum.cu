...
__global__ void ratio_weighted_sum_partial_kernel(const float* x, const float* y, float scale, float epsilon,
...
extern "C" int ratio_weighted_sum_run(float scale, float epsilon, const float* x, const float* y, float* out,
  ...
  ratio_weighted_sum_partial_kernel<<<blocks, kBlockSize>>>(x, y, scale, epsilon, partials, n);
