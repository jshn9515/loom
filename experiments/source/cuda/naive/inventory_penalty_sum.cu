...
__global__ void inventory_penalty_partial_kernel(const float* pos, float target, float* partials,
                                                 std::int64_t n) {
...
extern "C" int inventory_penalty_sum_run(float target, const float* pos, float* out, std::int64_t n) {
  ...
  inventory_penalty_partial_kernel<<<blocks, kBlockSize>>>(pos, target, partials, n);
