...
__global__ void weighted_dot_partial_kernel(const float* x, const float* y, float weight, float* partials,
                                            std::int64_t n) {
...
extern "C" int weighted_dot_run(float weight, const float* x, const float* y, float* out, std::int64_t n) {
  ...
  weighted_dot_partial_kernel<<<blocks, kBlockSize>>>(x, y, weight, partials, n);
