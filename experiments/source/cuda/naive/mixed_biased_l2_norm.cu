...
__global__ void mixed_biased_l2_norm_partial_kernel(const float* x, float scale, float bias,
...
extern "C" int mixed_biased_l2_norm_run(float scale, float bias, const float* x, float* out,
  ...
  mixed_biased_l2_norm_partial_kernel<<<blocks, kBlockSize>>>(x, scale, bias, partials, n);
