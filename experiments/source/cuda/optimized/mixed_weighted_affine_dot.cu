...
__global__ void mixed_weighted_affine_dot_partial_kernel(const float* __restrict__ x,
                                                         const float* __restrict__ y, float weight,
  ...
  for (std::int64_t i = idx; i < n; i += stride) {
    float transformed = scale * x[i] + bias;
    accum += weight * transformed * y[i];
  }
...
extern "C" std::size_t mixed_weighted_affine_dot_workspace_size(std::int64_t n) {
  return workspace_bytes(n);
}

extern "C" int mixed_weighted_affine_dot_run(float weight, float scale, float bias,
  ...
  mixed_weighted_affine_dot_partial_kernel<kBlockSize><<<blocks, kBlockSize>>>(
      x, y, weight, scale, bias, partials, n);
