...
__global__ void dot_partial_kernel(const float* __restrict__ x, const float* __restrict__ y,
...
extern "C" std::size_t dot_workspace_size(std::int64_t n) { return workspace_bytes(n); }

extern "C" int dot_run(const float* x, const float* y, float* out, std::int64_t n, void* workspace,
  ...
  dot_partial_kernel<kBlockSize><<<blocks, kBlockSize>>>(x, y, partials, n);
