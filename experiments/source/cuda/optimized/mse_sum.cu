...
__global__ void mse_partial_kernel(const float* __restrict__ x, const float* __restrict__ y,
...
extern "C" std::size_t mse_sum_workspace_size(std::int64_t n) { return workspace_bytes(n); }

extern "C" int mse_sum_run(const float* x, const float* y, float* out, std::int64_t n, void* workspace,
  ...
  mse_partial_kernel<kBlockSize><<<blocks, kBlockSize>>>(x, y, partials, n);
