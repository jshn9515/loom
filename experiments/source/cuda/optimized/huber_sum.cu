...
__global__ void huber_partial_kernel(const float* __restrict__ x, const float* __restrict__ y, float delta,
  ...
  for (std::int64_t i = idx; i < n; i += stride) {
    ...
    accum += abs_d > delta ? linear : quadratic;
  }
...
extern "C" std::size_t huber_sum_workspace_size(std::int64_t n) { return workspace_bytes(n); }

extern "C" int huber_sum_run(float delta, const float* x, const float* y, float* out, std::int64_t n,
  ...
  huber_partial_kernel<kBlockSize><<<blocks, kBlockSize>>>(x, y, delta, partials, n);
