...
__global__ void mixed_book_signal_kernel(const float* __restrict__ bid,
                                         const float* __restrict__ ask, float scale,
...
extern "C" int mixed_book_signal_run(float scale, float epsilon, float threshold,
                                     const float* bid, const float* ask, float* out,
                                     std::int64_t n) {
  if (n <= 0) {
    return 0;
  }
