...
__global__ void l2_partial_kernel(const float* __restrict__ x, float* __restrict__ partials,
                                  std::int64_t n) {
  float accum = 0.0f;
...
extern "C" std::size_t l2_norm_sq_workspace_size(std::int64_t n) { return workspace_bytes(n); }

extern "C" int l2_norm_sq_run(const float* x, float* out, std::int64_t n, void* workspace,
  ...
  l2_partial_kernel<kBlockSize><<<blocks, kBlockSize>>>(x, partials, n);
