...
__global__ void piecewise_weighted_dot_partial_kernel(const float* __restrict__ x, const float* __restrict__ y,
                                                      float weight_pos, float weight_neg,
  ...
  for (std::int64_t i = idx; i < n; i += stride) {
    float weight = x[i] > 0.0f ? weight_pos : weight_neg;
...
extern "C" std::size_t piecewise_weighted_dot_workspace_size(std::int64_t n) { return workspace_bytes(n); }

extern "C" int piecewise_weighted_dot_run(float weight_pos, float weight_neg, const float* x, const float* y,
  ...
  piecewise_weighted_dot_partial_kernel<kBlockSize><<<blocks, kBlockSize>>>(x, y, weight_pos, weight_neg, partials, n);
