...
__global__ void l2_partial_kernel(const float* x, float* partials, std::int64_t n) {
...
extern "C" int l2_norm_sq_run(const float* x, float* out, std::int64_t n) {
  if (n <= 0) {
  ...
  l2_partial_kernel<<<blocks, kBlockSize>>>(x, partials, n);
