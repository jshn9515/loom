...
let script_path () =
  match Sys.getenv_opt "LOOM_CPP_FRONTEND" with
  | Some path -> path
  | None -> Filename.concat (Sys.getcwd ()) "src/frontends/cpp/loom_frontend_cpp.py"
...
let run_frontend args =
  ...
  | Unix.WEXITED code ->
      Diagnostic.raise_error
        (Printf.sprintf "C++ frontend failed with exit status %d" code)
  | Unix.WSIGNALED signal ->
      Diagnostic.raise_error
        (Printf.sprintf "C++ frontend was killed by signal %d" signal)
  | Unix.WSTOPPED signal ->
      Diagnostic.raise_error
        (Printf.sprintf "C++ frontend was stopped by signal %d" signal)
...
let entry_param_kind_of_string = function
  ...
  | value ->
      Diagnostic.raise_error
        (Printf.sprintf "unknown C++ frontend entry parameter kind %s" value)
...
let field fields name =
  ...
  | None ->
      Diagnostic.raise_error
        (Printf.sprintf "missing C++ frontend JSON field %s" name)

let expect_assoc = function
  | `Assoc fields -> fields
  | _ -> Diagnostic.raise_error "expected C++ frontend JSON object"

let expect_string field = function
  | `String value -> value
  | _ ->
      Diagnostic.raise_error
        (Printf.sprintf "expected string for C++ frontend field %s" field)

let expect_list field = function
  | `List values -> values
  | _ ->
      Diagnostic.raise_error
        (Printf.sprintf "expected list for C++ frontend field %s" field)
